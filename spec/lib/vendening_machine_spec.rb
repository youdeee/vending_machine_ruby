...
describe VendingMachine do
  describe "Step0:" do
    it "有効なお金を入れると合計金額が見れる" do
      ...
      vm.input_money(Money.cash(10))
      vm.input_money(Money.cash(50))
      vm.input_money(Money.cash(10000))
  ...
  describe "Step1:" do
    it "想定外のお金を入れたらそのままお釣りが出る" do
      ...
      expect(vm.input_money(Money.cash(5))).to eq(5)
      expect(vm.inputted_money).to eq(0)
    end
  end
  ...
  describe "Step6:" do
    ...
    it "釣り銭用ストックを取得できる" do
      ...
      vm.input_money(Money.cash(1000))
  ...
  describe "Original1(電子マネー対応)" do
    it "電子マネーでコーラを購入できる" do
      vm = VendingMachine.new
      vm.input_money(Money.emoney(1234))
      changes = vm.show_changes

      expect { vm.buy("コーラ") }.to change { vm.drinks_count }.by(-1)
      expect(vm.inputted_money).to eq(1114)
      expect(vm.sales).to eq(120)
    ...
    it "電子マネーを入れた後に現金を入れると、電子マネーはキャンセルされる" do
      ...
      expect(vm.inputted_money).to eq(500)
    end
    ...
    it "電子マネーを入れた後に電子マネーを入れると、最初の電子マネーはキャンセルされる" do
      vm = VendingMachine.new
      vm.input_money(Money.emoney(1234))
      vm.input_money(Money.emoney(234))

      expect(vm.inputted_money).to eq(234)
    end
  end
end
